...
class SqliteService {
  ...
  Future<Database> get database async {
    ...
    _database = await openDatabase(p.join(root, _databaseName), version: _databaseVersion, onConfigure: (db) async => db.execute('PRAGMA foreign_keys = ON'), onCreate: (db, version) async { await _createSchema(db); }, onUpgrade: (db, oldVersion, newVersion) async { if (oldVersion < 2) await _createMasterSchema(db); });
    return _database!;
  }
  ...
  Future<void> replaceMasterData(Map<String, dynamic> datasets) async {
    ...
    await db.transaction((txn) async {
      for (final entry in datasets.entries) {
        ...
        await txn.delete('master_data_rows', where: 'dataset = ?', whereArgs: [entry.key]);
        final batch = txn.batch();
        for (var index = 0; index < rows.length; index++) {
          final row = rows[index];
          if (row is! Map) continue;
          final payload = Map<String, dynamic>.from(row);
          final key = _rowKey(payload, index);
          batch.insert('master_data_rows', {'dataset': entry.key, 'row_key': key, 'payload_json': jsonEncode(payload), 'synced_at': now}, conflictAlgorithm: ConflictAlgorithm.replace);
        }
        await batch.commit(noResult: true);
        await txn.insert('sync_metadata', {'key': entry.key, 'synced_at': now, 'row_count': rows.length, 'status': 'success', 'error_message': ''}, conflictAlgorithm: ConflictAlgorithm.replace);
      }
    });
  }
  ...
  Future<bool> hasMasterData() async {
    final db = await database;
    final result = await db.rawQuery('SELECT COUNT(DISTINCT dataset) AS total FROM master_data_rows');
    return ((result.first['total'] as int?) ?? 0) >= 5;
  }

  Future<DateTime?> lastMasterSync() async {
    final db = await database;
    final rows = await db.rawQuery('SELECT MAX(synced_at) AS value FROM master_data_rows');
    final value = rows.first['value']?.toString();
    return value == null ? null : DateTime.tryParse(value)?.toLocal();
  }

  Future<void> upsertUsers(Iterable<LocalUser> users) async {
    final list = users.toList(growable: false), db = await database;
    final now = DateTime.now().toUtc().toIso8601String();
    await db.transaction((txn) async { final batch = txn.batch(); for (final user in list) { final values = user.toMap()..remove('id'); values['updated_at'] = now; values['created_at'] = now; batch.insert('user_app_mobile', values, conflictAlgorithm: ConflictAlgorithm.replace); } await batch.commit(noResult: true); });
  }

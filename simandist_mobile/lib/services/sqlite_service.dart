...
class SqliteService {
  ...
  Future<void> _createSchema(Database db) async {
    await db.execute('''
      CREATE TABLE user_app_mobile (
        ...
        kode_uiw TEXT NOT NULL DEFAULT '', kode_up3 TEXT NOT NULL DEFAULT '',
        kode_ulp TEXT NOT NULL DEFAULT '', ulp TEXT NOT NULL DEFAULT '',
        username TEXT NOT NULL COLLATE NOCASE UNIQUE,
        password_hash TEXT NOT NULL, password_salt TEXT NOT NULL,
        role TEXT NOT NULL DEFAULT '', bidang TEXT NOT NULL DEFAULT '',
        tim TEXT NOT NULL DEFAULT '', sub_tim TEXT NOT NULL DEFAULT '',
        akses_menu TEXT NOT NULL DEFAULT '', is_active INTEGER NOT NULL DEFAULT 1,
        source_updated_at TEXT, synced_at TEXT NOT NULL,
    ...
    await db.execute('''CREATE TABLE sync_metadata (
      key TEXT PRIMARY KEY, remote_revision TEXT NOT NULL DEFAULT '', synced_at TEXT,
      row_count INTEGER NOT NULL DEFAULT 0, status TEXT NOT NULL DEFAULT 'idle',
  ...
  Future<void> upsertUsers(Iterable<LocalUser> users) async {
    ...
    await db.transaction((txn) async {
      ...
      await txn.insert('sync_metadata', {
        'key': 'User_App_Mobile', 'synced_at': now, 'row_count': list.length,
  ...
  Future<void> markSyncError(String key, Object error) async {
    final db = await database;
    await db.insert('sync_metadata', {'key': key, 'status': 'error', 'error_message': error.toString()}, conflictAlgorithm: ConflictAlgorithm.replace);
  }
  ...
  LocalUser _fromMap(Map<String, Object?> row) => LocalUser(
    id: row['id'] as int?, remoteNo: '${row['remote_no'] ?? ''}',
    kodeUiw: '${row['kode_uiw'] ?? ''}', kodeUp3: '${row['kode_up3'] ?? ''}',
    kodeUlp: '${row['kode_ulp'] ?? ''}', ulp: '${row['ulp'] ?? ''}',
    username: '${row['username'] ?? ''}', passwordHash: '${row['password_hash'] ?? ''}',
    passwordSalt: '${row['password_salt'] ?? ''}', role: '${row['role'] ?? ''}',
    bidang: '${row['bidang'] ?? ''}', tim: '${row['tim'] ?? ''}',
    subTim: '${row['sub_tim'] ?? ''}', aksesMenu: '${row['akses_menu'] ?? ''}',
    isActive: row['is_active'] == 1, sourceUpdatedAt: row['source_updated_at']?.toString(),

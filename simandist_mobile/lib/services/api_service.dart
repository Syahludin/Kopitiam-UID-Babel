...
class ApiService {
  ...
  static Future<Map<String, dynamic>> loginPerangkat(String username, String password) async {
    ...
    if (result['success'] == true && result['deviceToken'] != null) await DeviceSessionService.save(deviceToken: result['deviceToken'].toString(), profile: result);
    return result;
  }
  ...
  static Future<Map<String, dynamic>> getMasterData(String token) async {
    final uri = Uri.parse('$baseUrl?action=getMasterData&token=${Uri.encodeQueryComponent(token)}');
    return _decode(await http.get(uri).timeout(const Duration(seconds: 60)));
  }

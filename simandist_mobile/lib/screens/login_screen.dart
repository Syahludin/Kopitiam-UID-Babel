  ...
  Future<void> _handleLogin() async {
    ...
    try {
      ...
      if (result['success'] == true) {
        ...
        if (!mounted) return;
        for (final key in ['token', 'deviceToken', 'username', 'role', 'kodeUiw', 'kodeUp3', 'kodeUlp', 'ulp', 'bidang', 'tim', 'subTim', 'aksesMenu']) await prefs.setString(key, (result[key] ?? '').toString());
        if (!mounted) return;
        await LocalAuthService.saveAfterOnlineLogin(username: username, password: password, profile: result);
        if (!mounted) return;
        Navigator.pop(context);
        if (!mounted) return;
        Navigator.of(context).pushReplacement(MaterialPageRoute(builder: (_) => DashboardScreen(sesi: result)));
      } else {
      ...
      if (offline != null) {
        ...
        Navigator.pop(context);
        if (!mounted) return;
        Navigator.of(context).pushReplacement(MaterialPageRoute(builder: (_) => DashboardScreen(sesi: offline)));
      } else {

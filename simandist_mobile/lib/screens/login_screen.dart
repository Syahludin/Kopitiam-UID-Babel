...
class LoginScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Scaffold(
    ...
    body: SafeArea(child: Stack(children: [
      ...
      Center(child: SingleChildScrollView(padding: const EdgeInsets.symmetric(horizontal: 32), child: Column(mainAxisSize: MainAxisSize.min, children: [
        SizedBox(width: 132, height: 132, child: SvgPicture.asset('assets/icons/logo_app.svg')),
        const SizedBox(height: 20),
        const Text.rich(TextSpan(children: [TextSpan(text: 'SiMan', style: TextStyle(color: AppColors.navy900)), TextSpan(text: 'Dist', style: TextStyle(color: AppColors.amber600))]), style: TextStyle(fontSize: 40, fontWeight: FontWeight.w800)),
        const SizedBox(height: 4), const Text('Sistem Manajemen Distribusi', style: TextStyle(fontSize: 15, color: AppColors.neutral500)),
        const SizedBox(height: 20),
        Container(padding: const EdgeInsets.fromLTRB(8, 6, 12, 6), decoration: BoxDecoration(color: AppColors.navy100, borderRadius: BorderRadius.circular(100)), child: const Row(mainAxisSize: MainAxisSize.min, children: [Icon(Icons.location_on, size: 12, color: AppColors.amber600), SizedBox(width: 6), Text('PLN UID BABEL', style: TextStyle(fontSize: 11, fontWeight: FontWeight.w700, letterSpacing: .5, color: AppColors.navy700))])),
        const SizedBox(height: 44),
        SizedBox(width: 260, height: 54, child: ElevatedButton.icon(onPressed: () => showModalBottomSheet(context: context, isScrollControlled: true, backgroundColor: Colors.transparent, builder: (_) => const _LoginSheet()), icon: const Icon(Icons.login, size: 18), label: const Text('Login', style: TextStyle(fontSize: 16, fontWeight: FontWeight.w700)), style: ElevatedButton.styleFrom(backgroundColor: AppColors.navy700, foregroundColor: Colors.white, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(15)), elevation: 4))),
      ]))),
...
class _PlnBadge extends StatelessWidget { const _PlnBadge(); @override Widget build(BuildContext context) => Container(width: 42, height: 42, decoration: BoxDecoration(color: AppColors.plnYellow, borderRadius: BorderRadius.circular(10), boxShadow: [BoxShadow(color: Colors.black.withValues(alpha: .15), blurRadius: 8, offset: const Offset(0, 3))]), child: const Icon(Icons.bolt, color: AppColors.plnRed, size: 28)); }

class _LoginSheet extends StatefulWidget { const _LoginSheet(); @override State<_LoginSheet> createState() => _LoginSheetState(); }
class _LoginSheetState extends State<_LoginSheet> {
  final _userCtrl = TextEditingController(); final _passCtrl = TextEditingController();
  bool _obscure = true, _loading = false; String? _error;
  @override void dispose() { _userCtrl.dispose(); _passCtrl.dispose(); super.dispose(); }

  Future<void> _handleLogin() async {
    final username = _userCtrl.text.trim(), password = _passCtrl.text;
    if (username.isEmpty || password.isEmpty) { setState(() => _error = 'Username dan kata sandi wajib diisi.'); return; }
    ...
    try {
      ...
      if (result['success'] == true) {
        ...
        if (!mounted) return;
        for (final key in ['token','username','role','kodeUiw','kodeUp3','kodeUlp','ulp','bidang','tim','subTim','aksesMenu']) await prefs.setString(key, (result[key] ?? '').toString());
        if (!mounted) return;
        await LocalAuthService.saveAfterOnlineLogin(username: username, password: password, profile: result);
        if (!mounted) return;
        Navigator.pop(context);
        if (!mounted) return;
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Login berhasil, ${result['username']}')));
      } else { setState(() => _error = (result['message'] ?? 'Login gagal.').toString()); }
    } catch (_) {
      ...
      if (offline != null) {
        final prefs = await SharedPreferences.getInstance();
        if (!mounted) return;
        for (final entry in offline.entries) if (entry.key != 'success') await prefs.setString(entry.key, entry.value.toString());
        if (!mounted) return;
        Navigator.pop(context);
        if (!mounted) return;
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Mode offline aktif. Data lokal digunakan.')));
      } else { setState(() => _error = 'Tidak dapat terhubung ke API dan akun ini belum tersimpan untuk mode offline.'); }
    } finally { if (mounted) setState(() => _loading = false); }
  }

  @override Widget build(BuildContext context) => Padding(padding: EdgeInsets.only(bottom: MediaQuery.of(context).viewInsets.bottom), child: Container(decoration: const BoxDecoration(color: Colors.white, borderRadius: BorderRadius.vertical(top: Radius.circular(26))), child: Column(mainAxisSize: MainAxisSize.min, children: [
    ...
    Container(margin: const EdgeInsets.fromLTRB(12, 8, 12, 0), padding: const EdgeInsets.fromLTRB(18, 14, 18, 22), decoration: BoxDecoration(gradient: const LinearGradient(begin: Alignment.topLeft, end: Alignment.bottomRight, colors: [Color(0xFFF7FCFF), Colors.white]), borderRadius: BorderRadius.circular(20), border: Border.all(color: AppColors.navy100), boxShadow: [BoxShadow(color: AppColors.navy700.withValues(alpha: .08), blurRadius: 18, offset: const Offset(0, 6))]), child: SingleChildScrollView(child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [

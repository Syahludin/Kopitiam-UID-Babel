...
function doGet(e) {
  ...
  try {
    if (action === 'health') return json_({ success: true, service: 'SiManDist API', version: '2.3.2' });
...
function getWoInsjar_(token) {
  ...
  return { success: true, total: rows.length, totalSheet: validRows, kodeUlpFilter: userKodeUlp, ulpFilter: auth.sesi.ulp || '', rows: rows };
}
...
function syncWoInsjar_(token, rows) {
  ...
  try { lock.waitLock(20000); } catch (error) { return { success: false, message: 'Server sibuk. Coba lagi sebentar.' }; }
  try {
    var sheet = woSheet_();
...
function sha256_(value) {
  ...
  return bytes.map(function(byte) { var normalized = byte < 0 ? byte + 256 : byte; return ('0' + normalized.toString(16)).slice(-2); }).join('');
}

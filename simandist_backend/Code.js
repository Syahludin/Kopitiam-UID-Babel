...
function doGet(e) {
  ...
  try {
    ...
    if (action === "loginPerangkat")
      return json_(loginPerangkat_(p.username, p.password, p.perangkat));
    if (action === "cekPerangkat") return json_(cekPerangkat_(p.deviceToken));
    if (action === "logoutPerangkat")
      return json_(logoutPerangkat_(p.deviceToken, p.token));
...
function doPost(e) {
  try {
    ...
    if (action === "login" || action === "loginPerangkat")
      return json_(
        loginPerangkat_(body.username, body.password, body.perangkat),
      );
    if (action === "cekPerangkat")
      return json_(cekPerangkat_(body.deviceToken));
    if (action === "logoutPerangkat")
      return json_(logoutPerangkat_(body.deviceToken, body.token));
    ...
    if (action === "syncWoInsjar")
      return json_(syncWoInsjar_(body.token, body.rows));
    return json_({
      success: false,
      message: "Action API tidak dikenal: " + action,
    });
...
function loginPerangkat_(username, password, perangkat) {
  ...
  if (!row || String(row[USER_COL.password] || "") !== password)
...
function cekPerangkat_(deviceToken) {
  ...
  if (!row) {
    ...
    return {
      success: false,
      kode: "AKUN_TIDAK_ADA",
      message: "Akun sudah tidak terdaftar.",
    };
  }
  if (
    sha256_(String(row[USER_COL.password] || "")) !==
    ...
    return {
      ...
      message: "Password berubah. Silakan login ulang.",
    };
  }
...
function getMasterData_(token) {
  ...
  for (var i = 0; i < CONFIG.MASTER_SHEETS.length; i++) {
    ...
    if (!sheet)
      return { success: false, message: "Sheet tidak ditemukan: " + name };
    ...
    for (var r = 1; r < values.length; r++) {
      if (
        ...
        normalize_(values[r][USER_COL.username]) !== username
      )
        continue;
      var item = {};
      var hasValue = false;
      for (var c = 0; c < headers.length; c++) {
        ...
        if (name === CONFIG.USERS_SHEET && normalize_(key) === "password")
          continue;
        item[key] = values[r][c];
...
function woSheet_() {
  var sheet = SpreadsheetApp.openById(CONFIG.WO_SPREADSHEET_ID).getSheetByName(
    CONFIG.WO_INSJAR_SHEET,
  );
  if (!sheet)
    throw new Error("Sheet tidak ditemukan: " + CONFIG.WO_INSJAR_SHEET);
  return sheet;
}
...
function getWoInsjar_(token) {
  ...
  var values = sheet.getDataRange().getDisplayValues();
  if (values.length < 2)
    return { success: true, total: 0, totalSheet: 0, rows: [] };
  ...
  for (var h = 0; h < headers.length; h++)
    headerIndex[normalize_(headers[h])] = h;

  var kodeIndex = headerIndex["kode wo"];
  var kodeUlpIndex = headerIndex["kode ulp"];
  var ulpIndex = headerIndex["ulp"];
  if (kodeIndex === undefined)
    return {
      success: false,
      message: "Header Kode WO tidak ditemukan pada WO_Ins_Jar.",
    };
  ...
  for (var r = 1; r < values.length; r++) {
    ...
    var rowKodeUlp =
      kodeUlpIndex === undefined ? "" : normalizeCode_(values[r][kodeUlpIndex]);
    var rowUlp = ulpIndex === undefined ? "" : normalize_(values[r][ulpIndex]);
    var kodeFromWo = normalizeCode_(
      kodeWo.replace(/^INSJAR-/i, "").substring(0, userKodeUlp.length),
    );

    var matchCode =
      userKodeUlp && (rowKodeUlp === userKodeUlp || kodeFromWo === userKodeUlp);
    var matchName =
      userUlp &&
      rowUlp &&
      (rowUlp === userUlp ||
        rowUlp.indexOf(userUlp) >= 0 ||
        userUlp.indexOf(rowUlp) >= 0);
...
function syncWoInsjar_(token, rows) {
  ...
  if (auth.success !== true) return auth;
  if (!rows || !rows.length)
    return { success: true, diproses: 0, diperbarui: 0, ditambahkan: 0 };
  ...
  try {
    ...
    var kodeIndex = headerIndex["kode wo"];
    if (kodeIndex === undefined)
      throw new Error("Header Kode WO tidak ditemukan.");
...
function getUsersRows_() {
  ...
  if (!sheet)
    throw new Error("Sheet " + CONFIG.USERS_SHEET + " tidak ditemukan.");
  var rows = sheet.getDataRange().getDisplayValues();
  try {
    cache.put("users_v2", JSON.stringify(rows), 300);
  } catch (_) {}
  return rows;
}

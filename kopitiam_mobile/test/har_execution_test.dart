...
void main() {
  ...
  testWidgets('unified screen has two cards and three summary statuses', (tester) async { final repo = DemoHarRepo(); await tester.pumpWidget(MaterialApp(theme: KopitiamTheme.light, home: HarExecutionScreen(sesi: repo.session, repository: repo))); await tester.pumpAndSettle(); expect(find.text('RINGKASAN WORK ORDER'), findsOneWidget); for (final text in ['Belum dikerjakan', 'Progress', 'Selesai']) expect(find.text(text), findsOneWidget); });

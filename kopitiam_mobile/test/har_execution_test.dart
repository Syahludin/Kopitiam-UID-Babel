...
Future<void> openWorkTab(WidgetTester tester) async {
  await tester.tap(find.text('Work Order Har').last);
...
void main() {
  ...
  testWidgets('unified screen exposes the Work Order tab', (tester) async { final repo = DemoHarRepo(); await tester.pumpWidget(MaterialApp(theme: KopitiamTheme.light, home: HarExecutionScreen(sesi: repo.session, repository: repo))); await tester.pumpAndSettle(); expect(find.text('Work Order Har'), findsWidgets); });

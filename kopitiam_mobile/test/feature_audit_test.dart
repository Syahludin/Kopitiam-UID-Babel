...
void main() {
  group('Audit: Model & Logika WO', () {
    test('fromRemote dengan data dummy server dipetakan utuh', () {
      final wo = WoInsjar.fromRemote({
        ...
        'Koordinat Akhir': '-6.13, 106.11',
        'Realisasi kmS': '2.45',
        'Waktu Mulai': '08:00',
        'Waktu Selesai': '09:30',
        'Durasi Pekerjaan': '01:30:00',
        'Status WO': 'Dalam Pengerjaan',
      });
    ...
    test('normalisasiStatus menerima variasi status server', () {
      expect(
        WoInsjar.normalisasiStatus('SELESAI'),
        WoInsjar.statusSelesai,
      );
      expect(
        WoInsjar.normalisasiStatus('berjalan'),
        WoInsjar.statusDalam,
      );
      expect(
        WoInsjar.normalisasiStatus('Mulai Pengerjaan'),
    ...
    test('round-trip toMap <-> fromMap mempertahankan is_dirty & status', () {
      final wo = WoInsjar(
        ...
        statusWo: WoInsjar.statusSelesai,
        isDirty: true,
      );
    ...
    test('durasi, format tanggal, dan parse stamp round-trip', () {
      expect(
        WoInsjar.formatTanggal(DateTime(2026, 8, 1)),
        '01 Agustus 2026',
      );
      expect(
        WoInsjar.hitungDurasi(DateTime(2026, 8, 1, 8), DateTime(2026, 8, 1, 9, 30)),
        '01:30:00',
      );
  ...
  group('Audit: Model Temuan & Peta ke Backend', () {
    test('toRemote memuat semua kunci yang dibaca backend Apps Script', () {
      final item = TemuanInspeksi(
        kodeTemuan: 'PLG-2026-001.TO-001',
        kodeWo: 'PLG-2026-001',
        segmen: 'A-04/A-05',
        ...
        tinggiPohon: 10,
        prioritas: 'Mayor',
        koordinat: '-6.12, 106.34',
        fotoTemuan: 'a.jpg',
        fotoLingkungan: 'b.jpg',
      );
    ...
    test('round-trip dan fromRemote menormalkan kolom server', () {
      final source = TemuanInspeksi(
        kodeTemuan: 'PLG-2026-001.TO-002',
        kodeWo: 'PLG-2026-001',
        temuan: 'Kabel Geser',
        dirty: true,
      );
      ...
      final serverRow = TemuanInspeksi.fromRemote({
        'Kode WO': 'PLG-2026-001',
        'Kode Temuan': 'PLG-2026-001.TO-002',
        'Temuan': 'Kabel Geser',
        'Koordinat Temuan': '-6.12, 106.34',
        'Prioritas': 'Sedang',
        'Waktu Input': '29 Agustus 2026, 08:00:00',
      });
  ...
  group('Audit: Prioritas mengikuti formula IFS (data dummy master)', () {
    ...
    test('Pohon vegetasi: Minor (jarak>5 dan tinggi<9)', () {
      expect(
        repo.prioritas('Rabas / Pangkas', 6, 2.5, const []),
        'Minor',
      );
    });

    test('Pohon vegetasi: Sedang (jarak 2..6 dan tinggi>=9)', () {
      expect(
        repo.prioritas('Tebang Sedang', 4, 10, const []),
        'Sedang',
      );
    });

    test('Pohon vegetasi: Mayor (jarak<3 dan tinggi>=9)', () {
      expect(repo.prioritas('Tebang Besar', 2, 10, const []), 'Mayor');
      expect(repo.prioritas('Rabas / Pangkas', 0.5, 9, const []), 'Mayor');
    });

    test('Pohon vegetasi menutup celah kombinasi IFS (selalu ada prioritas)', () {
      expect(repo.prioritas('Rabas / Pangkas', 7, 12, const []), 'Minor');
      expect(repo.prioritas('Tebang Sedang', 1, 5, const []), 'Mayor');
      expect(repo.prioritas('Rabas / Pangkas', 3, 5, const []), 'Mayor');
      expect(repo.prioritas('Tebang Besar', 7, 8, const []), 'Minor');
      expect(repo.prioritas('Rabas/Pangkas', 4, 7, const []), 'Mayor');
      expect(repo.prioritas('  tebang sedang  ', 6, 10, const []), 'Minor');
    });

    test('Non-vegetasi: LOOKUP master via kolom Temuan maupun Nama Temuan', () {
      const master = [
        {'Temuan': 'Kabel Geser', 'Prioritas': 'Sedang'},
        {'Nama Temuan': 'Pohon Tersangkut', 'Prioritas': 'Mayor'},
        {'Nama Temuan': '', 'Prioritas': 'Minor'},
      ];
      expect(repo.prioritas('Kabel Geser', null, null, master), 'Sedang');
      expect(repo.prioritas('Pohon Tersangkut', null, null, master), 'Mayor');
    });

    test('Header master dengan varian nama kolom (normalisasi)', () {
      const master = [
        {' NAMA TEMUAN ': 'Baret Tiang', 'Prioritas ': 'Minor'},
      ];
      expect(repo.prioritas('Baret Tiang', null, null, master), 'Minor');
    });

    test('jenisObject menyesuaikan Sub-Tim', () {
      expect(repo.jenisObject({'subTim': 'Inspeksi Jaringan'}), 'Jaringan');
      expect(repo.jenisObject({'subTim': 'INSJAR'}), 'Jaringan');
      expect(repo.jenisObject({'tim': 'Inspeksi Gardu'}), 'Gardu');
      expect(repo.jenisObject({'subTim': 'Lain-lain'}), '');
    });
  });
  ...
  group('Audit: Watermark menempel pada foto (render 2048px)', () {
    test('hasil render adalah JPEG 2048px dengan panel terpasang', () async {
      ...
      final source = File(
        p.join(Directory.systemTemp.path, 'dummy_foto_temuan.jpg'),
      );
      ...
      final item = TemuanInspeksi(
        ...
        waktuInput: '29 Agustus 2026, 08:00:12',
      );
      final output = await PhotoWatermarkService.render(
      ...
      final panelBrightness = panelPixel.r + panelPixel.g + panelPixel.b;
      expect(panelBrightness, lessThan(220), reason: 'panel harus terpasang');
      ...
      expect(backgroundBrightness, greaterThan(680), reason: 'latar putih');
      source.deleteSync();
  ...
  group('Audit: Kontrak fitur aktif di kode sumber', () {
    test('ApiService merutekan semua action backend', () {
      ...
      expect(source, contains("'action': 'syncWoInsjar'"));
    ...
    test('baseUrl API menunjuk Apps Script https', () {
      final uri = Uri.parse(ApiService.baseUrl);
      expect(uri.scheme, 'https');
      expect(uri.host, 'script.google.com');
      expect(uri.path, contains('/macros/s/'));
      expect(uri.path, endsWith('/exec'));
    });

    test('sumber sinkron temuan mengirim payload + foto base64', () {
      final source = File('lib/services/temuan_repository.dart')
          .readAsStringSync();
      expect(source, contains('fotoTemuanBase64'));
      expect(source, contains('fotoLingkunganBase64'));
      expect(source, contains("'is_dirty': 0"));
      expect(source, contains('ApiService.syncTemuan(token, payload)'));
    });

    test('tidak ada implementasi fitur yang masih placeholder', () {
      ...
      for (final file in files) {
        ...
        expect(
          ...
          false,
          reason: '${file.path} masih punya stub UnimplementedError',
        );
      }
    });
  });

...
class _C4aFindingsHomeState extends State<C4aFindingsHome> {
  ...
  Future<void> _sync() async {
    ...
    if (mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('${result.berhasil} terkirim, ${result.gagal} gagal.')),
      );
      setState(() => busy = false);
    }
  }
  ...
  Future<void> _retry(TemuanInspeksi item) async {
    ...
    if (mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text(ok ? 'Temuan berhasil dikirim.' : 'Masih gagal, data lokal tetap aman.')),
      );
      setState(() => busy = false);
    }
  }
  ...
  Future<void> _add() async {
    ...
    final changed = await Navigator.push<bool>(
      context,
      MaterialPageRoute(builder: (_) => TemuanFormScreen.c4a(sesi: widget.sesi, repository: repo)),
    );
    if (changed == true) {
      await _load();
  ...
  Widget build(BuildContext context) {
    if (!RoleProvider.hasC4aAccess(widget.sesi)) return const C4aAccessDeniedScreen();
    return Scaffold(
      appBar: AppBar(
      ...
      body: RefreshIndicator(
        ...
        child: items.isEmpty
            ? ListView(children: const [
                ...
                SizedBox(height: 12),
                Center(child: Text('Belum ada draft C4A', style: TextStyle(fontWeight: FontWeight.w800))),
                Center(child: Text('Tekan + untuk mencatat temuan.', style: TextStyle(color: muted))),
              ])
    ...
    return Card(
      ...
      child: Padding(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Row(children: [
            Expanded(child: Text(item.kodeTemuan, style: const TextStyle(fontWeight: FontWeight.w800, color: blue))),
            Icon(synced ? Icons.cloud_done : failed ? Icons.error_outline : Icons.schedule, color: color, size: 18),
            const SizedBox(width: 5),
            Text(_statusLabel(item.syncStatus), style: TextStyle(color: color, fontWeight: FontWeight.w700)),
          ]),
          ...
          Row(children: [
            ...
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Text(item.temuan, style: const TextStyle(fontWeight: FontWeight.w800)),
              Text('${item.jenisObject} • ${item.penyulang} • ${item.prioritas}', style: const TextStyle(color: muted)),
            ])),
          ]),
          if (failed) ...[
            const SizedBox(height: 8),
            Text(item.syncError, maxLines: 2, overflow: TextOverflow.ellipsis, style: const TextStyle(color: red, fontSize: 12)),
            Align(
              alignment: Alignment.centerRight,
              child: TextButton.icon(onPressed: busy ? null : () => _retry(item), icon: const Icon(Icons.refresh), label: const Text('Kirim ulang')),
            ),
          ],
        ]),
      ),
  ...
  Widget _thumb(String path) => ClipRRect(
        ...
        child: path.isNotEmpty && File(path).existsSync()
            ...
            : Container(width: 64, height: 64, color: Colors.grey.shade200, child: const Icon(Icons.image_not_supported_outlined)),
      );

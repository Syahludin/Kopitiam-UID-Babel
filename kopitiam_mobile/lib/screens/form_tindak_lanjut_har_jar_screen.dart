...
class _FormTindakLanjutHarJarScreenState
  ...
  Future<void> _ambilGps() async {
    ...
    try {
      final fix = await HighAccuracyLocationService.acquire(
        onSample: (_, accuracy) {
          if (mounted)
            setState(() => _locAccuracy = fixAccuracyLabel(accuracy));
        },
      );
      if (mounted) {
        setState(() {
  ...
  Widget _temuanCard(WoHarJar wo) => _card('Review Temuan', [
    ...
    if (wo.linkFotoTemuan.isNotEmpty || wo.linkFotoTiangSekitar.isNotEmpty) ...[
      const SizedBox(height: 10),
    ...
    InkWell(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          borderRadius: BorderRadius.circular(14),
          border: Border.all(color: _fotoSesudah.isNotEmpty ? blue : line),
        ),
  ...
  Widget _koordinatCard() => _card('Koordinat Realisasi', [
    Container(
      width: double.infinity,
      padding: const EdgeInsets.all(12),
      decoration: BoxDecoration(
    ...
    SizedBox(
      ...
      child: OutlinedButton.icon(
        ...
        label: Text(
          _locating ? 'Mengambil GPS...' : 'Ambil Koordinat GPS',
  ...
  Widget _catatanCard() => _card('Catatan Petugas', [
    TextField(

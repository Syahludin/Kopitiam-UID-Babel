...
class _ActionButton extends StatefulWidget { final String label; final IconData icon; final bool primary; final WoAction? action; const _ActionButton({required this.label, required this.icon, required this.primary, this.action}); @override State<_ActionButton> createState() => _ActionButtonState(); }
class _ActionButtonState extends State<_ActionButton> { bool loading = false; bool success = false; Future<void> _run() async { if (loading || widget.action == null) return; setState(() { loading = true; success = false; }); try { await widget.action!(); if (mounted) setState(() { loading = false; success = true; }); await Future<void>.delayed(const Duration(milliseconds: 1400)); if (mounted) setState(() => success = false); } catch (_) { if (mounted) setState(() => loading = false); } } @override Widget build(BuildContext context) { final background = success ? KopitiamColors.success : widget.primary ? KopitiamColors.gold : KopitiamColors.navy; final foreground = widget.primary && !success ? KopitiamColors.ink : KopitiamColors.surface; return SizedBox(height: 49, child: ElevatedButton.icon(onPressed: loading ? null : _run, icon: loading ? SizedBox(width: 16, height: 16, child: CircularProgressIndicator(strokeWidth: 2, color: foreground)) : Icon(success ? Icons.check_rounded : widget.icon, color: foreground, size: 17), label: Text(success ? 'Selesai' : widget.label, style: TextStyle(color: foreground, fontWeight: FontWeight.w900, fontSize: 11)), style: ElevatedButton.styleFrom(backgroundColor: background, disabledBackgroundColor: background.withValues(alpha: .65), elevation: 0, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(13))))); } }

class _NetworkStatusChip extends StatelessWidget { final bool? online; const _NetworkStatusChip({required this.online}); @override Widget build(BuildContext context) { final color = online == true ? KopitiamColors.success : online == false ? KopitiamColors.danger : KopitiamColors.muted; final label = online == true ? 'ONLINE' : online == false ? 'OFFLINE' : 'CEK...'; return Semantics(label: 'Status jaringan $label. Ketuk untuk periksa ulang.', liveRegion: true, child: Container(key: const ValueKey('network-status-chip'), padding: const EdgeInsets.symmetric(horizontal: 9, vertical: 7), decoration: BoxDecoration(color: KopitiamColors.navy, borderRadius: BorderRadius.circular(100), border: Border.all(color: KopitiamColors.surface)), child: Row(mainAxisSize: MainAxisSize.min, children: [Container(key: const ValueKey('network-status-dot'), width: 9, height: 9, decoration: BoxDecoration(color: color, shape: BoxShape.circle)), const SizedBox(width: 7), Text(label, style: const TextStyle(color: KopitiamColors.surface, fontSize: 10, fontWeight: FontWeight.w900, letterSpacing: .55))]))); } }

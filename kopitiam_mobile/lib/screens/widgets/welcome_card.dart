...
class _WelcomeCardState extends State<WelcomeCard> with WidgetsBindingObserver {
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        boxShadow: const [BoxShadow(color: Color(0x33071F33), blurRadius: 22, offset: Offset(0, 10))],
      ),
        ...
        Padding(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            Container(
              ...
              decoration: BoxDecoration(color: KopitiamColors.navy, borderRadius: BorderRadius.circular(15), border: Border.all(color: KopitiamColors.gold.withValues(alpha: .35))),
              child: Row(children: [
                Container(width: 34, height: 34, decoration: BoxDecoration(color: KopitiamColors.gold, borderRadius: BorderRadius.circular(10)), child: const Icon(Icons.bolt_rounded, color: KopitiamColors.ink, size: 22)),
                const SizedBox(width: 11),
                Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  Text(_online == true ? 'Terhubung ke server' : _online == false ? 'Mode offline aktif' : 'Memeriksa jaringan', maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(color: KopitiamColors.surface, fontSize: 13, fontWeight: FontWeight.w900)),
                  const SizedBox(height: 2),
                  Text(_online == true ? 'Data siap diunduh dan disinkronkan' : _online == false ? 'Data lokal tetap aman di perangkat' : 'Mohon tunggu sebentar', maxLines: 2, overflow: TextOverflow.ellipsis, style: const TextStyle(color: KopitiamColors.surface, fontSize: 11, fontWeight: FontWeight.w600)),
                ])),
              ]),
            ),
          ]),
  ...
  Widget _info(IconData icon, String label, String value) => Row(children: [
    Container(width: 36, height: 36, decoration: BoxDecoration(color: KopitiamColors.navy, borderRadius: BorderRadius.circular(11), border: Border.all(color: KopitiamColors.gold.withValues(alpha: .45))), child: Icon(icon, size: 17, color: KopitiamColors.gold)),
    const SizedBox(width: 8),
    Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      Text(value.isEmpty ? '-' : value, maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(color: KopitiamColors.surface, fontSize: 12, fontWeight: FontWeight.w900)),
    ])),
  ]);
}
...
class _NetworkStatusChip extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final dotColor = online == true ? KopitiamColors.success : online == false ? KopitiamColors.danger : KopitiamColors.muted;
    ...
    return Semantics(
      ...
      child: Container(
        ...
        child: Row(mainAxisSize: MainAxisSize.min, children: [
          Container(key: const ValueKey('network-status-dot'), width: 9, height: 9, decoration: BoxDecoration(color: dotColor, shape: BoxShape.circle)),

...
class _WoInsduFormScreenState extends State<WoInsduFormScreen>
  ...
  static const numericLabels = <String, String>{
    ...
    'bebanUtamaSWbp': 'Beban Utama S (A) WBP',
    'bebanUtamaTWbp': 'Beban Utama T (A) WBP',
    'bebanJurusanNWbp': 'Beban Jurusan N (A) WBP',
    'teganganRswbp': 'Tegangan R-S (V) WBP',
    'teganganStwbp': 'Tegangan S-T (V) WBP',
    'teganganRtwbp': 'Tegangan R-T (V) WBP',
    'teganganRnwbp': 'Tegangan R-N (V) WBP',
    'teganganSnwbp': 'Tegangan S-N (V) WBP',
    'teganganTnwbp': 'Tegangan T-N (V) WBP',
    'bebanUtamaRLwbp': 'Beban Utama R (A) LWBP',
    'bebanUtamaSLwbp': 'Beban Utama S (A) LWBP',
    'bebanUtamaTLwbp': 'Beban Utama T (A) LWBP',
    'bebanJurusanNLwbp': 'Beban Jurusan N (A) LWBP',
    'teganganRslwbp': 'Tegangan R-S (V) LWBP',
    'teganganStlwbp': 'Tegangan S-T (V) LWBP',
    'teganganRtlwbp': 'Tegangan R-T (V) LWBP',
    'teganganRnlwbp': 'Tegangan R-N (V) LWBP',
    'teganganSnlwbp': 'Tegangan S-N (V) LWBP',
    'teganganTnlwbp': 'Tegangan T-N (V) LWBP',
  };
  ...
  Future<void> _openMaps() async {
    if (_coordinate.isEmpty) {
      _message('Koordinat Gardu belum tersedia.');
      return;
    }
    await launchUrl(
      Uri.parse(
  ...
  Future<void> _save() async {
    ...
    try {
      ...
      final saved = WoInsdu(
        ...
        waktuMulai: wo.waktuMulai.isEmpty
            ? WoInsjar.stampLengkap(start)
            : wo.waktuMulai,
        waktuSelesai: _finish ? WoInsjar.stampLengkap(now) : wo.waktuSelesai,
        durasiPekerjaan: _finish
            ? WoInsjar.hitungDurasi(start, now)
            : wo.durasiPekerjaan,
  ...
  Widget _detailTab() => ListView(
        padding: const EdgeInsets.all(16),
        children: [
          _card('Identitas Work Order', [
            _row('Kode WO', wo.kodeWo),
            _row('ULP', wo.ulp),
            _row('Tanggal', '${wo.hari}, ${wo.tanggal}'),
            _row('Penyulang', wo.penyulang),
            _row('Section', wo.section),
            _row('Nomor Gardu', wo.nomorGardu),
            _row('Jurusan', wo.jurusan),
            ...
            InkWell(
              ...
              child: Container(
                ...
                decoration: BoxDecoration(
                  ...
                  borderRadius: BorderRadius.circular(12),
                  border: Border.all(color: const Color(0xFFB8DCEF)),
                ),
                child: Row(
                  children: [
                    const Icon(Icons.directions_rounded, color: blue),
                    ...
                    Expanded(
                      child: Column(
                        ...
                        children: [
                          ...
                          Text(
                            _coordinate.isEmpty ? 'Belum tersedia' : _coordinate,
                            style: const TextStyle(color: muted, fontSize: 12),
                          ),
                        ],
                      ),
                    ),
                    const Icon(Icons.open_in_new_rounded, color: blue, size: 18),
                  ],
                ),
              ),
            ),
          ...
          _card('Status Pekerjaan', [
            _row('Status WO', WoInsdu.normalisasiStatus(wo.statusWo)),
            _row('Waktu Mulai', wo.waktuMulai),
            _row('Waktu Selesai', wo.waktuSelesai),
            _row('Durasi', wo.durasiPekerjaan),
          ]),
        ],
      );

  Widget _measurementTab() => Column(
        children: [
          Expanded(
            child: ListView(
              padding: const EdgeInsets.all(16),
              children: [
                _inputGroup('Pengukuran WBP', numericLabels.entries.take(10)),
                const SizedBox(height: 14),
                _inputGroup('Pengukuran LWBP', numericLabels.entries.skip(10)),
                const SizedBox(height: 14),
                _inputGroup('Kondisi Cover & Jumperan', conditionLabels.entries),
              ],
            ),
          ),
          if (!readOnly)

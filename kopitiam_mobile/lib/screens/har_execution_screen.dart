        ...
        children: [
          ...
          if (loadError != null)
            Padding(
              ...
              child: Text(loadError!, style: const TextStyle(color: KopitiamColors.danger)),
            ),
          Expanded(
            child: IndexedStack(
              index: selected,
        ...
        children: [
          Row(
            children: [
              ...
              Expanded(
                child: Text(
                  master.synced ? 'Master Data Sync' : 'Master Data belum sinkron',
                  style: const TextStyle(color: KopitiamColors.ink, fontSize: 18, fontWeight: FontWeight.w900),
                ),
              ),
            ],
          ),
          const SizedBox(height: 8),
          Text('Terakhir sync: $lastSync'),
          ...
          if (masterProgress != null) ...[
            ...
            Text('${((masterProgress ?? 0) * 100).round()}%'),
          ],
          const SizedBox(height: 18),
          SizedBox(
...
class _HarWoDetailScreenState extends State<HarWoDetailScreen> with SingleTickerProviderStateMixin {
  ...
  Future<void> _add() async {
    await Navigator.push<bool>(
      context,
      MaterialPageRoute(builder: (_) => HarJobFormScreen(item: item, repository: widget.repository)),
    );
    await _reload();
  }
  ...
  Widget _job(Map<String, dynamic> job) {
    final materials = item.materials.where((material) => material['Kode Pekerjaan'] == job['Kode Pekerjaan']).toList();
    return Padding(
      padding: const EdgeInsets.only(bottom: 20),
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Text('${job['Uraian Pekerjaan']}', style: Theme.of(context).textTheme.titleMedium),
          const SizedBox(height: 6),
          Text('${job['Jumlah']} ${job['Set']}'),
          Text('${job['Kode Pekerjaan']}', style: Theme.of(context).textTheme.bodySmall),
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        ...
        children: [
          RefreshIndicator(
            ...
            child: ListView(
              ...
              children: [
                ...
                ...item.header.entries.where((entry) => !['Foto Sesudah', 'Link Foto Sesudah', 'Catatan Petugas', 'Status WO'].contains(entry.key)).map((entry) => _field(entry.key, entry.value)),
                for (final key in ['Link Foto Temuan', 'Link Foto Tiang Sekitar'])
                ...
                Text('Foto Sesudah', style: Theme.of(context).textTheme.titleMedium),
                const SizedBox(height: 12),
                if (photo.isNotEmpty && File(photo).existsSync())
                  ClipRRect(borderRadius: BorderRadius.circular(16), child: Image.file(File(photo), height: 200, fit: BoxFit.cover)),
                if (!item.finished)
                  OutlinedButton.icon(
                    onPressed: busy ? null : _photo,
                    icon: const Icon(Icons.camera_alt_outlined),
                    label: Text(photo.isEmpty ? 'Ambil foto' : 'Ambil ulang foto'),
                  ),
                if (item.value('Link Foto Sesudah').isNotEmpty)
                  TextButton(onPressed: () => _link(item.value('Link Foto Sesudah')), child: const Text('Buka foto tersinkron')),
                const SizedBox(height: 16),
                TextField(controller: notes, enabled: !item.finished && !busy, maxLines: 3, decoration: const InputDecoration(labelText: 'Catatan Petugas')),
                const SizedBox(height: 20),
                FilledButton(
                  onPressed: item.finished || busy ? null : _finish,
                  child: Text(busy ? 'Menyimpan...' : item.finished ? 'WO Selesai' : 'Simpan Selesai'),
                ),
          ...
          RefreshIndicator(
            ...
            child: ListView(
              ...
              children: [
                Text('${item.jobs.length} pekerjaan', style: Theme.of(context).textTheme.titleLarge),
                const SizedBox(height: 16),
      ...
      floatingActionButton: tabs.index == 1 && !item.finished
          ? FloatingActionButton(tooltip: 'Tambah pekerjaan', onPressed: busy ? null : _add, child: const Icon(Icons.add))
          : null,
    );
  }
}

...
class _HarExecutionScreenState extends State<HarExecutionScreen> {
  ...
  Future<void> _start(HarExecution item) async {
  ...
  Future<void> _open(HarExecution item) async {
    if (busy || item.ready) return;
    final current = await repo.get(item.type, item.code) ?? item;
    if (!mounted) return;
    await Navigator.push<void>(
      context,
      MaterialPageRoute(
        builder: (_) => HarWoDetailScreen(item: current, repository: repo),
      ),
    );
    await _refresh();
  }
          ...
          children: [
            ...
            ...items.map(
              (item) => HarExecutionCard(
                item: item,
                onStart: busy ? null : () => _start(item),
                onOpen: busy ? null : () => _open(item),
              ),
            ),
          ],
        ),
                ...
                children: [
                  ...
                  if (photo.isNotEmpty && File(photo).existsSync())
                    ClipRRect(
                      ...
                      child: Image.file(photo.isEmpty ? File('') : File(photo), height: 200, fit: BoxFit.cover),
                    ),
                  if (!item.finished)
                    OutlinedButton.icon(
                  ...
                  TextField(
                    ...
                    maxLines: 3,
                    decoration: const InputDecoration(labelText: 'Catatan Petugas'),
                  ),
                  const SizedBox(height: 20),
                  FilledButton(

...
class _HarExecutionScreenState extends State<HarExecutionScreen>{
  ...
  Future<void> _load()async{if(type.isEmpty)return;final data=await repo.list(type);if(mounted)setState(()=>items=data);}
  ...
  Future<void> _tap(HarExecution item)async{
    ...
    if(!item.started){
      ...
      if(yes==true)await _run(()async{await repo.start(type,item.code);return 'WO dimulai. Klik ulang card untuk membuka detail.';});
      return;
    }
  ...
  Widget build(BuildContext context)=>Scaffold(
    ...
    body:types.isEmpty?const Center(child:Text('Tidak memiliki akses WO Har.')):Column(children:[
      if(types.length>1)Padding(padding:const EdgeInsets.all(16),child:SegmentedButton<String>(segments:types.map((t)=>ButtonSegment(value:t,label:Text(t))).toList(),selected:{type},onSelectionChanged:busy?null:(v){setState(()=>type=v.single);_load();})),
...
class _HarWoDetailScreenState extends State<HarWoDetailScreen> with SingleTickerProviderStateMixin{
  ...
  Future<void> _reload()async{final next=await widget.repository.get(item.type,item.code);if(next!=null&&mounted)setState(()=>item=next);}
  void _message(String text){if(mounted)ScaffoldMessenger.of(context).showSnackBar(SnackBar(content:Text(text)));}
  ...
  Future<void> _link(String url)async{final uri=Uri.tryParse(url);if(uri==null||uri.scheme!='https'){_message('Link belum tersedia.');return;}await launchUrl(uri,mode:LaunchMode.externalApplication);}
  @override
  Widget build(BuildContext context)=>Scaffold(
    ...
    body:TabBarView(controller:tabs,children:[
      ListView(padding:const EdgeInsets.all(18),children:[
        ...
        ...item.header.entries.where((e)=>!['Foto Sesudah','Link Foto Sesudah','Catatan Petugas','Status WO'].contains(e.key)).map((e)=>Padding(padding:const EdgeInsets.only(bottom:12),child:Column(crossAxisAlignment:CrossAxisAlignment.start,children:[Text(e.key,style:Theme.of(context).textTheme.bodySmall),SelectableText('${e.value??''}')])),
      ...
      ListView(padding:const EdgeInsets.fromLTRB(18,18,18,100),children:[
        ...
        ...item.jobs.map((job){final materials=item.materials.where((m)=>m['Kode Pekerjaan']==job['Kode Pekerjaan']).toList();return Padding(padding:const EdgeInsets.only(bottom:20),child:Column(crossAxisAlignment:CrossAxisAlignment.start,children:[Text('${job['Uraian Pekerjaan']}',style:Theme.of(context).textTheme.titleMedium),const SizedBox(height:6),Text('${job['Jumlah']} ${job['Set']}'),Text('${job['Kode Pekerjaan']}',style:Theme.of(context).textTheme.bodySmall),...materials.map((m)=>ListTile(contentPadding:EdgeInsets.zero,leading:const Icon(Icons.inventory_2_outlined),title:Text('${m['Material']}'),subtitle:Text('${m['Kepemilikan']}'),trailing:Text('${m['Jumlah']} ${m['Satuan']}'))),if(materials.isEmpty)const Text('Tanpa material'),const Divider()]));}),
      ]),
    ]),

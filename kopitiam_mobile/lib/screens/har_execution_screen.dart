...
class _HarExecutionScreenState extends State<HarExecutionScreen> {
  ...
  Widget _home() => RefreshIndicator(
        ...
        child: ListView(padding: const EdgeInsets.all(18), children: [
          ...
          _summary(),
          const SizedBox(height: 20),
          if (!admin) ElevatedButton.icon(onPressed: busy ? null : _downloadWo, icon: const Icon(Icons.download_rounded), label: const Text('Download WO')),
          if (!admin) const SizedBox(height: 12),
          FilledButton.icon(onPressed: busy ? null : () => _run(repo.syncAll), icon: const Icon(Icons.sync_rounded), label: const Text('Sinkron WO')),
        ]),
      );

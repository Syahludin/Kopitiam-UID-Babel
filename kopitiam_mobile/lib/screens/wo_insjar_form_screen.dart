...
class _WoInsjarFormScreenState extends State<WoInsjarFormScreen> {
  ...
  Future<void> _save() async {
    ...
    try {
      ...
      if (complete) {
        ...
        _message('Data Progress Pekerjaan Tersimpan');
      }
    } catch (error) {
  ...
  Future<void> _showFinishedDialog() => showDialog<void>(
        context: context,
  ...
  void _message(String text, {bool error = false}) {
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text(text),
        backgroundColor: error ? Colors.red.shade700 : ink,
      ),
    );
  }
        ...
        children: [
          Expanded(
            child: ListView(
              ...
              children: [
                _hero(wo),
                _section(1, 'Area inspeksi', _area(wo)),
                _section(2, 'Koordinat Pekerjaan', _coordinates()),
                _section(
                  3,
                  'Temuan inspeksi',
                  SizedBox(
                    height: 360,
                    child: TemuanTab(
                      key: ValueKey('temuan-${wo.kodeWo}-${_awal != null}'),
                      wo: wo,
                      sesi: widget.sesi,
                      canAddTemuan: _awal != null && !_readOnly,
                    ),
                  ),
                ),
                _section(4, 'Ringkasan pekerjaan', _summary()),
              ],
            ),
          ),
          if (!_readOnly)
              ...
              child: SizedBox(
                ...
                child: ElevatedButton(
                  ...
                  style: ElevatedButton.styleFrom(
                    elevation: 0,
                    backgroundColor: _readyToComplete ? yellow : navy,
                    foregroundColor: _readyToComplete ? ink : surface,
  ...
  Widget _hero(WoInsjar wo) => Container(
        clipBehavior: Clip.antiAlias,
        ...
        child: Stack(
          children: [
            const Positioned(
              right: -34,
              top: -54,
              child: _GoldRing(size: 118),
            ),
            const Positioned(
              right: 34,
              top: 18,
              child: _GoldRing(size: 72, opacity: .4),
            ),
            Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
  ...
  Widget _heroStatus() {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        boxShadow: const [
          BoxShadow(color: Color(0x33071F33), blurRadius: 16, offset: Offset(0, 7)),
        ],
      ),
      child: Row(
        mainAxisSize: MainAxisSize.min,
        children: [
          Icon(done ? Icons.check_circle_rounded : Icons.circle,
              size: done ? 13 : 7, color: done ? success : blueLight),
          const SizedBox(width: 6),
          Text(
            done ? 'Selesai' : 'Sedang dikerjakan',
            style: TextStyle(
              color: done ? success : blue,
              fontSize: 10,
              fontWeight: FontWeight.w900,
            ),
          ),
  ...
  Widget _section(int number, String title, Widget child) => Container(
        ...
        child: Column(
          ...
          children: [
            ...
            Padding(padding: const EdgeInsets.only(left: 39), child: child),
          ],
        ),
      );

  Widget _area(WoInsjar wo) => Container(
        clipBehavior: Clip.antiAlias,
        decoration: BoxDecoration(
          color: surface,
        ...
        child: Column(
          children: [
            _areaRow('PENYULANG', wo.penyulang),
            const Divider(height: 1, color: line),
            _areaRow(
              'SECTION',
              [wo.sectionAwal, wo.sectionAkhir]
    ...
    return AnimatedOpacity(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          border: Border.all(
            color: fix != null ? const Color(0xFF86CFA5) : line,
          ),
        ),
        child: Column(
          children: [
            Row(
              children: [
                _CoordinateRadar(loading: loading, done: fix != null),
                    ...
                    children: [
                      ...
                      Text(
                        loading
                            ...
                            : fix?.coordinate ??
                                (locked ? 'Menunggu koordinat awal' : 'Belum diambil'),
                        maxLines: 1,
              ...
              Row(
                children: [
                  const Text(
                    ...
                    style: TextStyle(color: blueLight, fontSize: 10, fontWeight: FontWeight.w800),
                  ),
                  const Spacer(),
                  Text(
                    liveAccuracy == null
                        ? 'Menunggu...'
                        : '${liveAccuracy.toStringAsFixed(1)} m',
            ...
            if (!_readOnly) ...[
              ...
              SizedBox(
                ...
                child: OutlinedButton(
                  onPressed: locked || loading || _saving
                      ? null
  ...
  Widget _summary() => GridView.count(
        ...
        children: [
          _metric('Realisasi kmS', '${_kms.toStringAsFixed(3)} km'),
          _metric('Durasi', _duration),
          _metric(
            'Waktu mulai',
            _mulai == null ? 'Belum tersedia' : WoInsjar.stampLengkap(_mulai!),
          ),
          _metric(
            'Waktu selesai',
            _selesai == null
                ? 'Belum tersedia'
                : WoInsjar.stampLengkap(_selesai!),
          ),
        ],
      );
...
class _GoldRing extends StatelessWidget {
  ...
  const _GoldRing({required this.size, this.opacity = .72});
  ...
  Widget build(BuildContext context) => Opacity(
        ...
        child: Container(
          ...
          decoration: BoxDecoration(
            shape: BoxShape.circle,
            border: Border.all(color: _WoInsjarFormScreenState.gold),
          ),
        ),
      );
}

class _CoordinateRadar extends StatelessWidget {
  final bool loading;
  final bool done;
  const _CoordinateRadar({required this.loading, required this.done});
          ...
          children: [
            if (loading)
              const SizedBox.square(
                ...
                child: CircularProgressIndicator(
                  strokeWidth: 2,
                  color: _WoInsjarFormScreenState.blueLight,
                ),
              ),
            AnimatedContainer(
              duration: const Duration(milliseconds: 180),
              ...
              decoration: BoxDecoration(
                color: done
                    ...
                    : loading
                        ? _WoInsjarFormScreenState.blueLight
                        : _WoInsjarFormScreenState.soft,
                borderRadius: BorderRadius.circular(loading ? 18 : 12),
              ),
              child: Icon(
                Icons.my_location_rounded,
                size: 18,
                color: done
                    ? _WoInsjarFormScreenState.success
                    : loading
                        ? _WoInsjarFormScreenState.surface
                        : _WoInsjarFormScreenState.blueLight,
              ),
            ),
          ],

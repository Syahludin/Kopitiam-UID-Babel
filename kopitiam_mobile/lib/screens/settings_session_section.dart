...
class _SettingsSessionSectionState extends State<SettingsSessionSection> {
  Timer? _expiryTimer; bool _loggingOut=false,_downloadingGardu=false;
  ...
  Future<void> _downloadMasterGardu()async{
    if(_downloadingGardu)return;setState(()=>_downloadingGardu=true);
    try{final response=await ApiService.getMasterGardu('${widget.session['token']??''}');if(response['success']!=true||response['rows'] is! List)throw StateError('${response['message']??'Master Gardu tidak valid.'}');final rows=response['rows'] as List;final db=await SqliteService.instance.database;final now=DateTime.now().toUtc().toIso8601String();await db.transaction((txn)async{await txn.delete('master_data_rows',where:'dataset = ?',whereArgs:['Master_Gardu']);for(var i=0;i<rows.length;i++){final row=rows[i];if(row is! Map)continue;await txn.insert('master_data_rows',{'dataset':'Master_Gardu','row_key':'$i','payload_json':jsonEncode(Map<String,dynamic>.from(row)),'synced_at':now},conflictAlgorithm:ConflictAlgorithm.replace);}});if(mounted)await showOperationResultDialog(context,success:true,title:'Master Gardu Tersimpan',message:'${rows.length} data gardu siap digunakan offline.');}catch(e){if(mounted)await showOperationResultDialog(context,success:false,title:'Download Master Gardu Gagal',message:'$e');}finally{if(mounted)setState(()=>_downloadingGardu=false);}
  }
  ...
  @override Widget build(BuildContext context){final offline=widget.session['offlineLogin']==true;return Column(children:[
    if(widget.showMasterGardu)Card(margin:const EdgeInsets.only(top:14),child:Padding(padding:const EdgeInsets.all(18),child:Column(crossAxisAlignment:CrossAxisAlignment.start,children:[const Text('Master Gardu',style:TextStyle(fontSize:17,fontWeight:FontWeight.w800)),const SizedBox(height:6),const Text('Diunduh terpisah agar master utama lebih ringan.'),const SizedBox(height:16),SizedBox(width:double.infinity,child:FilledButton.icon(onPressed:_downloadingGardu?null:_downloadMasterGardu,icon:const Icon(Icons.domain_rounded),label:Text(_downloadingGardu?'Mengunduh...':'Download Master Gardu')))]))),

...
class _SettingsSessionSectionState extends State<SettingsSessionSection> {
  ...
  void initState() {
    super.initState();
    _expiryTimer = Timer.periodic(const Duration(minutes: 1), (_) => _enforceOfflineExpiry());
    WidgetsBinding.instance.addPostFrameCallback((_) => _enforceOfflineExpiry());
  }

  @override
  void dispose() { _expiryTimer?.cancel(); super.dispose(); }
  ...
  Future<void> _downloadMasterGardu() async {
    ...
    try {
      ...
      setState(() => _garduProgress = rows.isEmpty ? 1 : 0);
    ...
    return Column(children: [
            ...
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              if (_garduProgress != null) ...[
                Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                  const Text('Menyimpan data gardu...', style: TextStyle(color: KopitiamColors.muted, fontSize: 12)),
                  Text('${(_garduProgress! * 100).round()}%', style: const TextStyle(color: KopitiamColors.navy, fontSize: 12, fontWeight: FontWeight.w800)),
                ]),
      ...
      Card(
        ...
        child: Padding(
          padding: const EdgeInsets.all(18),
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            const Text('Akun & Sesi', style: TextStyle(fontSize: 17, fontWeight: FontWeight.w800)),
            const SizedBox(height: 6),
            Text(offline ? 'Mode offline aktif, maksimal 24 jam sejak verifikasi online.' : 'Keluar akan mencabut token perangkat dan akses offline.'),
            const SizedBox(height: 16),
            SizedBox(width: double.infinity, child: OutlinedButton.icon(onPressed: _loggingOut ? null : _logout, icon: const Icon(Icons.logout_rounded), label: Text(_loggingOut ? 'Keluar...' : 'Log out'))),
          ]),
        ),
      ),
    ]);

...
typedef TemuanSender = Future<Map<String, dynamic>> Function(
  String token,
...
class C4aSyncResult {
  final int berhasil;
  final int gagal;
  const C4aSyncResult({this.berhasil = 0, this.gagal = 0});
}
...
class TemuanRepository {
  ...
  final TemuanSender sender;

  TemuanRepository({this.database, TemuanSender? sender})
      : sender = sender ?? ApiService.syncTemuan;
  ...
  Future<Database> _database() async {
    ...
    await db.execute('''CREATE TABLE IF NOT EXISTS temuan_inspeksi (
      ...
      kode_wo TEXT NOT NULL DEFAULT '', kode_uiw TEXT DEFAULT '',
      kode_up3 TEXT DEFAULT '', kode_ulp TEXT DEFAULT '', ulp TEXT DEFAULT '',
      hari TEXT DEFAULT '', tanggal TEXT DEFAULT '', penyulang TEXT DEFAULT '',
      section_awal TEXT DEFAULT '', section_akhir TEXT DEFAULT '', section TEXT DEFAULT '',
      segmen TEXT DEFAULT '', nomor_gardu TEXT DEFAULT '', koordinat TEXT DEFAULT '',
      lat TEXT DEFAULT '', long TEXT DEFAULT '', jenis_object TEXT DEFAULT '',
      tier TEXT DEFAULT '', temuan TEXT DEFAULT '', jarak REAL,
      jenis_pohon TEXT DEFAULT '', tinggi_pohon REAL, prioritas TEXT DEFAULT '',
      pekerjaan TEXT DEFAULT '', jenis_wo TEXT DEFAULT '', foto_temuan TEXT DEFAULT '',
      foto_lingkungan TEXT DEFAULT '', link_foto TEXT DEFAULT '', link_lingkungan TEXT DEFAULT '',
      waktu_input TEXT DEFAULT '', user_input TEXT DEFAULT '', folder_path TEXT DEFAULT '',
      is_dirty INTEGER DEFAULT 1, sync_status TEXT NOT NULL DEFAULT 'queued',
      sync_error TEXT NOT NULL DEFAULT '', retry_count INTEGER NOT NULL DEFAULT 0,
  ...
  Future<void> _addColumn(Database db, String name, String definition) async {
    ...
    if (!info.any((row) => '${row['name']}'.toLowerCase() == name)) {
      await db.execute('ALTER TABLE temuan_inspeksi ADD COLUMN $name $definition');
    }
  }
  ...
  /// Task 19: query tunggal untuk beranda C4A, opsional per status.
  Future<List<TemuanInspeksi>> daftarC4a({String? status}) async {
    final db = await _database();
    final filter = status?.trim().toLowerCase();
    final rows = await db.query(
      'temuan_inspeksi',
      where: filter == null || filter.isEmpty
          ? "kode_wo = ''"
          : "kode_wo = '' AND sync_status = ?",
      whereArgs: filter == null || filter.isEmpty ? null : [filter],
      orderBy: 'waktu_input DESC, kode_temuan DESC',
    );
    return rows.map(TemuanInspeksi.fromMap).toList();
  }
  ...
  Future<TemuanInspeksi> simpanC4a(TemuanInspeksi draft, {DateTime? savedAt}) async {
    validateC4a(draft);
    await _validateLandscapePhoto(draft.fotoTemuan);
    await _validateLandscapePhoto(draft.fotoLingkungan);
    final db = await _database();
    return db.transaction((txn) async {
      ...
      final item = TemuanInspeksi.fromMap({
        ...
        'is_dirty': 1,
        'sync_status': TemuanInspeksi.statusQueued,
        'sync_error': '',
        'retry_count': 0,
        'last_attempt_at': '',
        'link_foto': '',
  ...
  Future<void> _validateLandscapePhoto(String path) async {
    await _validatePhoto(File(path));
  ...
  Future<TemuanInspeksi> _preparePhotos(TemuanInspeksi item) async {
    ...
    final primarySource = item.kodeWo.isNotEmpty && _sudahWatermark(item.fotoTemuan)
        ? item.fotoTemuan
        : await PhotoWatermarkService.render(sourcePath: item.fotoTemuan, item: item, photoLabel: 'Foto Temuan');
    final environmentSource = item.kodeWo.isNotEmpty && _sudahWatermark(item.fotoLingkungan)
        ? item.fotoLingkungan
        : await PhotoWatermarkService.render(sourcePath: item.fotoLingkungan, item: item, photoLabel: 'Foto Lingkungan');
    final primary = await _persistPhoto(primarySource, item.kodeTemuan, 'Foto Temuan');
    final environment = await _persistPhoto(environmentSource, item.kodeTemuan, 'Foto Lingkungan');
    return TemuanInspeksi.fromMap({...item.toMap(), 'foto_temuan': primary.path, 'foto_lingkungan': environment.path});
  }

  Future<File> _persistPhoto(String sourcePath, String findingCode, String label) async {
    final source = File(sourcePath);
    await _validatePhoto(source);
    final root = await getApplicationDocumentsDirectory();
    final folder = Directory(p.join(root.path, 'temuan_photos', _safeName(findingCode)));
    await folder.create(recursive: true);
    final originalName = p.basename(source.path);
    final match = RegExp(r'(\d{6})(?=\.jpe?g$)', caseSensitive: false).firstMatch(originalName);
    final stamp = match?.group(1) ?? DateTime.now().toIso8601String().replaceAll(RegExp(r'[^0-9]'), '').substring(8, 14);
    final destination = File(p.join(folder.path, '${_safeName(findingCode)}.$label.$stamp.jpg'));
  ...
  Future<void> unduh(String token, String kodeWo) async {
    ...
    for (final row in response['rows'] as List) {
      if (row is Map) {
        await db.insert('temuan_inspeksi', TemuanInspeksi.fromRemote(Map<String, dynamic>.from(row)).toMap(), conflictAlgorithm: ConflictAlgorithm.ignore);
      }
    }
  }
  ...
  Future<Map<String, dynamic>> _payload(TemuanInspeksi item) async {
    ...
    return item.toRemote()
      ...
      ..['fotoLingkunganBase64'] = base64Encode(await environment.readAsBytes());
  }

  /// Tasks 17,20,21,22,24: durable, sequential, validated and idempotent queue.
  Future<C4aSyncResult> sinkronC4a(String token, {bool hanyaGagal = false}) async {
    final db = await _database();
    final rows = await db.query(
      'temuan_inspeksi',
      where: hanyaGagal
          ? "kode_wo = '' AND is_dirty = 1 AND sync_status = ?"
          : "kode_wo = '' AND is_dirty = 1 AND sync_status IN (?,?,?,?)",
      whereArgs: hanyaGagal
          ? [TemuanInspeksi.statusFailed]
          : [TemuanInspeksi.statusDraft, TemuanInspeksi.statusQueued, TemuanInspeksi.statusFailed, TemuanInspeksi.statusSending],
      orderBy: 'waktu_input, kode_temuan',
    );
    var ok = 0, failed = 0;
    for (final row in rows) {
      final item = TemuanInspeksi.fromMap(row);
      final attempt = DateTime.now().toUtc().toIso8601String();
      ...
      try {
        final response = await sender(token, await _payload(item));
        if (response['success'] != true) {
          final code = '${response['kode'] ?? ''}';
          final message = '${response['message'] ?? 'Sinkronisasi gagal.'}';
          if (code.startsWith('SESSION_')) throw StateError('$message [$code]');
          throw StateError(message);
        }
        await db.update('temuan_inspeksi', {
          'is_dirty': 0,
          'sync_status': TemuanInspeksi.statusSynced,
          'sync_error': '',
          'link_foto': '${response['linkFoto'] ?? ''}',
          'link_lingkungan': '${response['linkLingkungan'] ?? ''}',
          'folder_path': '${response['folderPath'] ?? item.folderPath}',
          'last_attempt_at': attempt,
        }, where: 'kode_temuan = ?', whereArgs: [item.kodeTemuan]);
        ok++;
      } catch (error) {
        await db.update('temuan_inspeksi', {
          'is_dirty': 1,
          'sync_status': TemuanInspeksi.statusFailed,
          'sync_error': error.toString().replaceFirst('Bad state: ', ''),
  ...
  /// Tasks 18,25: explicit retry service for one failed finding.
  Future<bool> kirimUlangC4a(String token, String kodeTemuan) async {
    final db = await _database();
    await db.update('temuan_inspeksi', {'sync_status': TemuanInspeksi.statusQueued, 'sync_error': ''}, where: "kode_temuan = ? AND kode_wo = ''", whereArgs: [kodeTemuan]);
    final rows = await db.query('temuan_inspeksi', where: 'kode_temuan = ?', whereArgs: [kodeTemuan], limit: 1);
    if (rows.isEmpty) return false;
    final item = TemuanInspeksi.fromMap(rows.first);
    try {
      final response = await sender(token, await _payload(item));
      if (response['success'] != true) throw StateError('${response['message'] ?? 'Sinkronisasi gagal.'}');
      await db.update('temuan_inspeksi', {
        'is_dirty': 0, 'sync_status': TemuanInspeksi.statusSynced, 'sync_error': '',
        'link_foto': '${response['linkFoto'] ?? ''}', 'link_lingkungan': '${response['linkLingkungan'] ?? ''}',
        'folder_path': '${response['folderPath'] ?? item.folderPath}',
        'last_attempt_at': DateTime.now().toUtc().toIso8601String(),
      }, where: 'kode_temuan = ?', whereArgs: [kodeTemuan]);
      return true;
    } catch (error) {
      await db.update('temuan_inspeksi', {
        'is_dirty': 1, 'sync_status': TemuanInspeksi.statusFailed,
        'sync_error': error.toString().replaceFirst('Bad state: ', ''),
        'retry_count': item.retryCount + 1,
        'last_attempt_at': DateTime.now().toUtc().toIso8601String(),
      }, where: 'kode_temuan = ?', whereArgs: [kodeTemuan]);
  ...
  /// Existing WO findings keep their old endpoint behavior.
  Future<void> sinkron(String token) async {
    final db = await _database();
    final rows = await db.query('temuan_inspeksi', where: "is_dirty = 1 AND kode_wo <> ''");
    for (final row in rows) {
      final item = TemuanInspeksi.fromMap(row);
      final primary = File(item.fotoTemuan), environment = File(item.fotoLingkungan);
      await _validatePhoto(primary); await _validatePhoto(environment);
      final payload = item.toRemote()
        ..['Foto Temuan'] = p.basename(primary.path)
        ..['Foto Lingkungan Sekitaran Tiang'] = p.basename(environment.path)
        ..['fotoTemuanBase64'] = base64Encode(await primary.readAsBytes())
        ..['fotoLingkunganBase64'] = base64Encode(await environment.readAsBytes());
      final response = await sender(token, payload);
      if (response['success'] != true) throw StateError('${response['message'] ?? 'Sinkronisasi foto gagal.'}');
  ...
  Future<List<Map<String, dynamic>>> master(String dataset) async {
    final db = await _database();
    final rows = await db.query('master_data_rows', columns: ['payload_json'], where: 'dataset = ?', whereArgs: [dataset]);
    return rows.map((row) => Map<String, dynamic>.from(jsonDecode('${row['payload_json']}'))).toList();
  }

  String jenisObject(Map<String, dynamic> sesi) {
    final value = '${sesi['subTim'] ?? sesi['tim'] ?? ''}'.toLowerCase();
    if (value.contains('inspeksi jaringan') || value.contains('insjar')) return 'Jaringan';
    if (value.contains('inspeksi gardu') || value.contains('insdu')) return 'Gardu';
    return '';
  }
  ...
  String prioritasC4a(String temuan, double? jarak, double? tinggi, List<Map<String, dynamic>> master) {
    if (isRowC4a(temuan)) {
      if (jarak == null || tinggi == null || !jarak.isFinite || !tinggi.isFinite || jarak < 0 || tinggi <= 0) return '';
      if (tinggi >= jarak * math.sqrt(2)) return 'Mayor';
      return prioritas(temuan, jarak, tinggi, master);
    }
    final values = master.where((r) => _same(_find(r, ['Temuan']), temuan)).map((r) => _find(r, ['Prioritas'])).where((v) => v.isNotEmpty).toSet();
    return values.length == 1 ? values.single : '';
  }

  String prioritas(String temuan, double? jarak, double? tinggi, List<Map<String, dynamic>> master) {
    if (isRowC4a(temuan)) {
      final distance = jarak ?? 0, height = tinggi ?? 0;
    ...
    for (final row in master) {
      final name = _find(row, const ['Temuan', 'Nama Temuan']);
      if (name.isNotEmpty && _same(name, temuan)) return _find(row, const ['Prioritas']);
    }
    return '';
  }

  static String _find(Map<String, dynamic> row, List<String> keys) {
    String normal(String key) => key.toLowerCase().replaceAll(RegExp(r'[^a-z0-9]'), '');
    for (final key in keys) { final value = '${row[key] ?? ''}'.trim(); if (value.isNotEmpty) return value; }
    final normalized = keys.map(normal).toSet();
    for (final entry in row.entries) if (normalized.contains(normal(entry.key)) && '${entry.value ?? ''}'.trim().isNotEmpty) return '${entry.value}'.trim();
    return '';
  }
  ...
  static String _folder(String ulp, String object, String leaf, DateTime now) {
    ...
    return 'Kopitiam/Rekap Temuan Inspeksi/$ulp/$object/${now.year}/${now.month.toString().padLeft(2, '0')}. ${months[now.month - 1]}/${now.day.toString().padLeft(2, '0')}/$leaf/';
  }
}

/// Task 17/21: while the C4A home is alive, detect recovered connectivity and flush.
class C4aSyncService {
  ...
  C4aSyncService({required this.repository, required this.token, this.interval = const Duration(seconds: 15), Future<bool> Function()? online})
      : online = online ?? _hasNetwork;

  void start({void Function(C4aSyncResult result)? onResult}) {
    _timer?.cancel();
    _timer = Timer.periodic(interval, (_) => flush(onResult: onResult));
    unawaited(flush(onResult: onResult));
  }

  Future<void> flush({void Function(C4aSyncResult result)? onResult}) async {
    if (_running || token.isEmpty || !await online()) return;
    _running = true;
    try {
      final result = await repository.sinkronC4a(token);
  ...
  static Future<bool> _hasNetwork() async {
    try {
      final result = await InternetAddress.lookup('script.google.com').timeout(const Duration(seconds: 4));

...
class WoHarDuRepository {
  ...
  Future<List<WoHarDu>> semua() async {
    ...
    final rows = await db.rawQuery(
      "SELECT * FROM ${DatabaseHelper.woHarDuTable} ORDER BY CASE status_wo "
      "WHEN '${WoHarDu.statusMenunggu}' THEN 0 WHEN '${WoHarDu.statusSedang}' "
      "THEN 1 ELSE 2 END, kode_wo DESC",
    );
  ...
  Future<WoSyncResult> download(String token) async {
    ...
    if (rows.isEmpty) {
      ...
      if (sheet > 0 && rejected > 0) {
        return WoSyncResult(
          total: 0,
          diproses: 0,
          pesan: 'Ada $sheet WO Har Du di server, tetapi $rejected tidak cocok '
              'dengan Kode ULP akun (${response['kodeUlpFilter'] ?? '-'} vs '
              '${response['sampleKodeUlp'] ?? '-'}).',
        );
      }
    }
    final db = await _db.database;
    ...
    await db.transaction((txn) async {
      for (final raw in rows) {
        ...
        if (status == WoHarDu.statusSelesai || status == WoHarDu.statusTersinkron) {
          continue;
        }
        available++;
  ...
  Future<void> simpanSelesai(
    ...
    final storedPhoto = await photo.copy(
      p.join(folder.path, '${_safe(source.kodeWo)}.${DateTime.now().millisecondsSinceEpoch}.jpg'),
    );
    ...
    final values = source.toMap()
      ...
      ..addAll({
        ...
        'user_input': username,
        'waktu_input': source.waktuInput.isEmpty ? WoInsjar.stampLengkap(now) : source.waktuInput,
  ...
  Future<WoSyncResult> sinkron(String token) async {
    ...
    if (response['success'] != true) {
      return WoSyncResult(
        total: rows.length,
        diproses: 0,
        pesan: '${response['message'] ?? 'Sinkronisasi WO Har Du gagal.'}',
      );
    }
    final processed = (response['diproses'] as num?)?.toInt() ?? 0;
    if (processed != rows.length) {
      return WoSyncResult(
        total: rows.length,
        diproses: processed,
        pesan: 'Konfirmasi server belum lengkap. Data lokal dipertahankan.',
      );
    }
    await db.transaction((txn) async {
      for (final row in rows) {
    ...
    return WoSyncResult(total: rows.length, diproses: processed);
  }

...
class WoHarJarRepository {
  ...
  Future<List<WoHarJar>> semua() async {
    ...
    final rows = await db.rawQuery(
      "SELECT * FROM ${DatabaseHelper.woHarJarTable} "
      "ORDER BY CASE status_wo "
      "WHEN '${WoHarJar.statusMenunggu}' THEN 0 "
      "WHEN '${WoHarJar.statusSedang}' THEN 1 "
  ...
  Future<int> jumlahBelumSinkron() async {
    ...
    final result = await db.rawQuery(
      "SELECT COUNT(*) AS total FROM ${DatabaseHelper.woHarJarTable} "
      "WHERE is_synced = 0 AND status_wo = '${WoHarJar.statusSelesai}'",
    );
  ...
  Future<WoSyncResult> download(String token) async {
    ...
    if (response['success'] != true || pendingList is! List) {
      return WoSyncResult(
        total: 0,
        diproses: 0,
        pesan: (response['message'] ?? 'Download WO Har Jar gagal.').toString(),
      );
    }
    final db = await _database();
    ...
    await db.transaction((txn) async {
      for (final row in pendingList) {
        ...
        if (status == WoHarJar.statusSelesai ||
            status == WoHarJar.statusTersinkron) {
          continue;
        }
        final inserted = await txn.insert(
          DatabaseHelper.woHarJarTable,
          item.copyWith(statusWo: WoHarJar.statusMenunggu).toMap(),
  ...
  Future<void> mulaiPekerjaan(String kodeWo) async {
    ...
    await db.update(
      DatabaseHelper.woHarJarTable,
      {
        'status_wo': WoHarJar.statusSedang,
        'waktu_input': WoInsjar.stampLengkap(DateTime.now()),
      },
      where: 'kode_wo = ? AND status_wo = ?',
      whereArgs: [kodeWo, WoHarJar.statusMenunggu],
    );
  }

  String generateKodeMaterial(String kodeWo) {
  ...
  TemuanInspeksi _watermarkItem(WoHarJar item) => TemuanInspeksi(
  ...
  Future<File> _persistPhoto(String sourcePath, String kodeWo) async {
    ...
    final folder = Directory(
      p.join(root.path, 'har_jar_photos', _safeName(kodeWo)),
    );
  ...
  Future<WoSyncResult> sinkron(String token) async {
    ...
    final rows = await db.query(
      DatabaseHelper.woHarJarTable,
      where:
          "is_synced = 0 AND status_wo = '${WoHarJar.statusSelesai}'",
    );
    if (rows.isEmpty) {
      return const WoSyncResult(
        total: 0,
        diproses: 0,
        pesan: 'Belum ada WO Har Jar selesai yang siap disinkronkan.',
      );
    }
    ...
    if (response['success'] != true) {
      return WoSyncResult(
        ...
        diproses: 0,
        pesan:
            (response['message'] ?? 'Sinkronisasi WO Har Jar gagal.').toString(),
      );
    }

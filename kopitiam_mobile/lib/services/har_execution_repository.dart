...
class HarExecutionRepository {
  ...
  Future<void> finish(String type,String code,String photo,String notes) async {
    ...
    final watermark = TemuanInspeksi(kodeTemuan:item.value('Kode Temuan'),kodeWo:item.code,jenisObject:item.value('Jenis Object'),temuan:item.value('Temuan'),ulp:item.value('ULP'),penyulang:item.value('Penyulang'),section:item.value('Section'),segmen:item.value('Segmen'),nomorGardu:item.value('Nomor Gardu'),koordinat:item.value('Koordinat'),waktuInput:stamp,userInput:owner); final rendered = await PhotoWatermarkService.render(sourcePath:photo,item:watermark,photoLabel:'Foto Sesudah'); final root = await getApplicationDocumentsDirectory(); final dir = Directory(p.join(root.path,'har_execution_photos',owner.replaceAll(RegExp(r'[^A-Za-z0-9._-]'),'_'))); await dir.create(recursive:true); final target = await File(rendered).copy(p.join(dir.path,'${HarExecution.newId('HAR')}.jpg')); final db = await _db(); await db.transaction((txn) async { final current = await _get(txn,type,code); if (current == null || current.finished) throw StateError('WO sudah selesai.'); await _put(txn,current.change(header:{...current.header,'Status WO':HarExecution.done,'Waktu Selesai':stamp,'User Input':owner,'Catatan Petugas':notes.trim()},photo:target.path,dirty:true,revision:current.revision+1)); });
  }

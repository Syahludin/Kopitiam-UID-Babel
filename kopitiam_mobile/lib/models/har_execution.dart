...
class HarExecution {
  ...
  bool get ready => !started && !finished;
  ...
  static bool isAdmin(Map<String, dynamic> session) => '${session['role'] ?? ''}'.trim().toLowerCase().contains('admin');
  static List<String> allowedTypes(Map<String, dynamic> session) {
    final team = '${session['subTim'] ?? session['tim'] ?? ''}'.trim().toLowerCase();
    final parts = '${session['username'] ?? ''}'.toLowerCase().split(RegExp(r'[.\s_-]+'));
    if (['har jar', 'harjar', 'har jaringan'].contains(team) || parts.contains('harjar')) return [jar];
    if (['har du', 'hardu', 'har gardu'].contains(team) || parts.contains('hardu')) return [du];
    if (isAdmin(session) || ['har', 'hartek', 'har teknik'].contains(team) || parts.contains('hartek') || parts.contains('har')) return [jar, du];
    return [];
  }
}

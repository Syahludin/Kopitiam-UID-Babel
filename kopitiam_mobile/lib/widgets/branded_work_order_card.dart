...
class BrandedWorkOrderCard extends StatelessWidget {
  ...
  final String code, typeLabel, status, finding, feeder, section, date, coordinate;
  final List<WorkOrderPhoto> photos;
  final bool waiting, finished;
  final VoidCallback onStart, onOpen;

  const BrandedWorkOrderCard({super.key, required this.code, required this.typeLabel, required this.status, required this.finding, required this.feeder, required this.section, required this.date, required this.coordinate, required this.photos, required this.waiting, required this.finished, required this.onStart, required this.onOpen});

  Future<void> _openLocation(BuildContext context) async {
    if (coordinate.trim().isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Lokasi Work Order belum tersedia.')));
      return;
    }
    final uri = Uri.parse('https://www.google.com/maps/dir/?api=1&destination=${Uri.encodeComponent(coordinate.trim())}');
  ...
  Future<void> _start(BuildContext context) async {
    final confirmed = await showWorkOrderStartDialog(
      ...
      detail: [feeder, section].where((value) => value.trim().isNotEmpty).join(' • '),
    );
  ...
  Widget _locationButton(BuildContext context) => SizedBox(
    ...
    child: Align(alignment: Alignment.topRight, child: Material(
      ...
      child: InkWell(
        ...
        borderRadius: BorderRadius.circular(100),
        child: const SizedBox(height: 32, child: Padding(
  ...
  Widget _details() => Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
    IntrinsicWidth(child: Container(
      ...
      decoration: BoxDecoration(color: findingSurface, borderRadius: BorderRadius.circular(12), border: Border.all(color: const Color(0xFFE8CF91))),
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        Text(finding.trim().isEmpty ? 'Temuan belum tersedia' : finding, maxLines: 2, overflow: TextOverflow.ellipsis, style: const TextStyle(color: ink, fontSize: 16, height: 1.2, fontWeight: FontWeight.w900)),
      ]),
    )),
    const SizedBox(height: 8),
    Text(feeder.trim().isEmpty ? 'Penyulang belum tersedia' : feeder, maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(color: ink, fontSize: 12, fontWeight: FontWeight.w800)),
    const SizedBox(height: 4),
    Text(section.trim().isEmpty ? 'Section belum tersedia' : section, maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(color: muted, fontSize: 11)),
  ]);
  ...
  Widget _primaryAction(BuildContext context) => SizedBox(
    ...
    child: waiting
      ...
      : OutlinedButton(
          onPressed: onOpen,
          style: OutlinedButton.styleFrom(foregroundColor: logoBlue, side: const BorderSide(color: logoBlue), padding: const EdgeInsets.symmetric(horizontal: 16), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(13))),
  ...
  Widget _photo(BuildContext context, WorkOrderPhoto photo) => Material(
    ...
    child: InkWell(
      ...
      child: AspectRatio(aspectRatio: 2.15, child: Stack(fit: StackFit.expand, children: [
        _image(photo.source),
  ...
  Widget _dateRow() => Column(children: [
    const SizedBox(height: 12),
    const Divider(height: 1, color: line),
    const SizedBox(height: 10),
    Row(children: [
  ...
  Widget _image(String source) {
    ...
    if (uri != null && (uri.scheme == 'http' || uri.scheme == 'https')) {
      return Image.network(path, fit: BoxFit.cover, errorBuilder: (_, __, ___) => const Center(child: Icon(Icons.broken_image_outlined, color: muted)));
    }
  ...
  Future<void> _showPhoto(BuildContext context, WorkOrderPhoto photo) async {
    await showDialog<void>(context: context, builder: (dialogContext) => Dialog(
      backgroundColor: ink,
      insetPadding: const EdgeInsets.all(20),
      shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(20)),
      child: Padding(padding: const EdgeInsets.all(14), child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.stretch, children: [
        Row(children: [
        ...
        ClipRRect(borderRadius: BorderRadius.circular(14), child: AspectRatio(aspectRatio: 4 / 3, child: _image(photo.source))),
      ])),
    ));
  }
}

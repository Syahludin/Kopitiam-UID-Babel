...
function accountStatus_(username) {
  ...
  for (var row = 1; row < values.length; row++) {
    if (normalize_(values[row][usernameIndex]) !== normalize_(username))
      continue;
...
function validateFindingMaster_(object, tier, finding, priority) {
  ...
  if (!sheet)
    return fail_("MASTER_SHEET_MISSING", "Master temuan tidak tersedia.");
  var values = sheet.getDataRange().getDisplayValues();
  if (values.length < 2) return fail_("MASTER_EMPTY", "Master temuan kosong.");
  var index = headerIndex_(values[0]);
  var findingIndex =
    index.temuan !== undefined ? index.temuan : index["nama temuan"];
  var objectIndex =
    index["objek inspeksi"] !== undefined
      ? index["objek inspeksi"]
      : index["jenis object"];
  ...
  for (var row = 1; row < values.length; row++) {
    ...
    if (
      normalize_(rowFinding) !== normalize_(finding) ||
      normalize_(rowTier) !== normalize_(tier) ||
      (rowObject && normalize_(rowObject).indexOf(normalize_(object)) < 0)
    )
      continue;
  ...
  return fail_(
    "FINDING_MASTER_INVALID",
    "Kombinasi Object, Tier, dan Temuan tidak terdaftar.",
  );
}

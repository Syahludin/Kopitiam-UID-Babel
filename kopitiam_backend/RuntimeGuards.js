...
function canonicalRole_(value) {
  var role = normalizeRole_(value);
  if (role === "superuser") return "Super User";
  if (role === "super user") return "Super User";
  if (role === "admin") return "Admin";
  if (role === "pegawai pln") return "Pegawai PLN";
  return String(value == null ? "" : value).trim();
}

function isC4aRoleAllowed_(value) {
  return C4A_ALLOWED_ROLES_.indexOf(normalizeRole_(value)) >= 0;
}

/** Role selalu dibaca ulang dari User_App_Mobile, bukan dari payload perangkat. */
function activeRoleProfile_(session) {
  if (!session || typeof session !== "object") {
    return fail_("SESSION_INVALID", "Sesi tidak valid.");
  }
  var username = String(session.username || "").trim();
  if (!username) return fail_("ACCOUNT_INACTIVE", "Akun tidak ditemukan.");
  var row = findUser_(username), status = accountStatus_(username);
  if (!row || !status.exists || !status.active) {
    return fail_("ACCOUNT_INACTIVE", "Akun tidak aktif atau tidak ditemukan.");
  }
  var profile = userFromRow_(row);
  profile.role = canonicalRole_(profile.role);
  profile.canAccessC4a = isC4aRoleAllowed_(row[USER_COL.role]);
  return {
    success: true,
    profile: profile,
    role: profile.role,
...
function requireC4aAccess_(session) {
  var profile = activeRoleProfile_(session);
  if (!profile.success) return profile;
  if (!profile.canAccessC4a) {
    return fail_("C4A_ACCESS_DENIED", "Role akun tidak memiliki akses C4A.");
  }
  return profile;
}
...
function isVegetasiFinding_(finding) {
  ...
  return VEGETASI_TEMUAN_.some(function (value) {
...
function validateFindingMaster_(object, tier, finding, priority) {
  ...
  var objectIndex = index["objek inspeksi"] !== undefined ? index["objek inspeksi"] : index["jenis object"];
  var tierIndex = index.tier;
  var priorityIndex = index.prioritas;
  if (findingIndex === undefined || tierIndex === undefined)
    return fail_("MASTER_HEADERS_INVALID", "Header Master_Temuan belum valid.");
  for (var row = 1; row < values.length; row++) {
    var rowFinding = String(values[row][findingIndex] || "").trim();
    var rowTier = String(values[row][tierIndex] || "").trim();
    var rowObject = objectIndex === undefined ? "" : String(values[row][objectIndex] || "").trim();
    if (normalize_(rowFinding) !== normalize_(finding) || normalize_(rowTier) !== normalize_(tier) || (rowObject && normalize_(rowObject).indexOf(normalize_(object)) < 0)) continue;
    var expected = priorityIndex === undefined ? "" : String(values[row][priorityIndex] || "").trim();
    if (expected && !isVegetasiFinding_(finding) && normalize_(expected) !== normalize_(priority))

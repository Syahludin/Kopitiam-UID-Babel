...
test("device token rejects absolute age, idle age, corruption, and future time", () => {
  ...
  assert.equal(
    api.validateDeviceRecord_(
      { createdAt: now - day, lastUsedAt: now - 1 },
      now,
    ),
    "",
  );
  assert.equal(
    api.validateDeviceRecord_(
      { createdAt: now - 7 * day, lastUsedAt: now - 1 },
      now,
    ),
    "DEVICE_MAX_AGE",
  );
  assert.equal(
    api.validateDeviceRecord_(
      { createdAt: now - 2 * day, lastUsedAt: now - day },
      now,
    ),
    "DEVICE_IDLE_EXPIRED",
  );
  ...
  assert.equal(
    api.validateDeviceRecord_({ createdAt: now + 1, lastUsedAt: now + 1 }, now),
    "DEVICE_TIME_INVALID",
  );
});
...
test("sensitive action quota fails closed after its limit", () => {
  ...
  for (let i = 0; i < 6; i++) {
    assert.equal(
      api.consumeActionQuota_("syncTemuanInspeksi", "device-a").success,
      true,
    );
  }
  ...
  assert.equal(
    api.consumeActionQuota_("syncTemuanInspeksi", "device-b").success,
    true,
  );
});

test("unknown actions do not consume a quota bucket", () => {
  const api = load();

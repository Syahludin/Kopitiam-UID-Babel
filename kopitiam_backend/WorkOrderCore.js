...
function woCoreSheet_(name) {
  var sheet = SpreadsheetApp.openById(CONFIG.WO_SPREADSHEET_ID).getSheetByName(name);
...
function woCoreAccess_(session, mode) {
  ...
  return { success: true, kodeUlp: kodeUlp, subTim: sub };
}
...
function woCoreGet_(token, mode, sheetName) {
  ...
  var sheet = woCoreSheet_(sheetName);
  ...
  for (var row = 1; row < values.length; row++) {
    ...
    if (normalizeCode_(values[row][index['kode ulp']]) !== access.kodeUlp) continue;
    if (mode === 'row' && teamIndex !== undefined) {
      var rowTeam = normalize_(values[row][teamIndex]);
      if (rowTeam && rowTeam !== access.subTim) continue;
    }
...
var WO_CORE_MUTABLE = {
  ...
  insdu: [
    'beban utama r (a) wbp', 'beban utama s (a) wbp', 'beban utama t (a) wbp', 'beban jurusan n (a) wbp',
    'tegangan r-s (v) wbp', 'tegangan s-t (v) wbp', 'tegangan r-t (v) wbp', 'tegangan r-n (v) wbp', 'tegangan s-n (v) wbp', 'tegangan t-n (v) wbp',
    'beban utama r (a) lwbp', 'beban utama s (a) lwbp', 'beban utama t (a) lwbp', 'beban jurusan n (a) lwbp',
    'tegangan r-s (v) lwbp', 'tegangan s-t (v) lwbp', 'tegangan r-t (v) lwbp', 'tegangan r-n (v) lwbp', 'tegangan s-n (v) lwbp', 'tegangan t-n (v) lwbp',
    'cover fco atas', 'cover fco bawah', 'cover bushing tm', 'cover bushing tr', 'cover arrester', 'jumperan atas', 'jumperan bawah',
    'waktu mulai', 'waktu selesai', 'durasi pekerjaan', 'status wo'
  ]
};
